...
# --------------------------------------------------------------------
def cut(event):
    return (event.n_lep_signal != 0) or (event.n_lep != 0) or (event.n_jet < 4) or (event.n_bjet < 1) or (event.jet1_pt < 100000) or (event.jet2_pt < 60000) (event.jet3_pt < 60000) or (event.jet4_pt < 40000) or (event.bjet1_pt < 50000) or (event.bjet2_pt > 50000) or (event.met < 250000) or (event.dPhimin_4 < 0.5) or (event.PassMetTrigger != 1) or (event.PassTruthMetFilter != 1) or (event.mT_b_MET_xMult_0_001 < 180) or (event.n_JetR12_WTagged < 1)
# ---------------------------------------------------------------------
def main():
#    NAME = 'Test'
#    NAME = 'n_WTagged1'
...
#	if s == 0:
#	    Z = -20.0
#	elif (sqrt(cuts.sqrdWeight_signal)/s + 0.2*s > 0.7):
#	    Z = -20.0
#	ntuple.read(row)
#	if ntuple("dPhimin_all") > 1.3: Z = -20.0
	if b < 20: Z = -20.0
	if (Z > bestZ):
	    bestZ = Z
	    bestrow = row
